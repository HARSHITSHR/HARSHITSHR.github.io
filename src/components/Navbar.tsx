        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navItems.map((item, index) => (
              ...
              >
                {item.label}
                <span className="absolute bottom-0 left-1/2 w-0 h-0.5 bg-accent transition-all duration-300 -translate-x-1/2 group-hover:w-full hover:w-full" />
              </motion.button>
            ))}
          </div>
            ...
            <div className="px-4 py-4 space-y-1">
              {navItems.map((item, index) => (
                <motion.button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className="block w-full text-left px-4 py-3 text-sm font-medium text-primary-foreground/80 hover:text-accent hover:bg-primary-foreground/5 rounded-lg transition-all uppercase tracking-wider"
